...
def build(output):
    ...
    story += [P("Universe selection", "Sub"),
              P(f"The demonstration directly selected {len(selection)} well-known, genuinely liquid large- and mid-cap F&amp;O names: RELIANCE, HDFCBANK, ICICIBANK, TCS, INFY, SBIN, AXISBANK, KOTAKBANK, LT, TATAMOTORS, BAJFINANCE, and ADANIENT. They span banks, IT, energy, industrials, autos, and NBFCs; ADANIENT and TATAMOTORS deliberately add high-idiosyncratic-volatility stress. Before the first trade, the engine checked option participation on {selection[0]['formation_date']}: at least 250 contracts traded, 1 million summed share open interest, and 15 rows with positive volume and open interest. All 12 passed; exact readings are in <tt>data/universe_selection.csv</tt>.", "Body2"),
              P("<b>Selection filter.</b> The documented live rule ranks NSE F&amp;O names by 20-day average option turnover, contract volume, and lot-adjusted open interest, then forms a ten-name liquid core subject to diversification constraints: no more than five banks, no more than two IT names, at least one energy, industrial, and NBFC name, and no more than two names from one corporate group. TATAMOTORS and ADANIENT are two declared stress overrides and must still clear minimum volume and open-interest eligibility. INDUSINDBK is omitted once the five-bank allocation is filled; TATASTEEL is omitted because TCS and the TATAMOTORS stress sleeve fill the two-name Tata-group cap. The present backtest uses the resulting fixed demonstration list and validates its first-day liquidity rather than replaying the complete historical ranking, so selection and survivorship bias remain.", "Callout")]
    ...
    coverage_rows = [["Assignment item", "Report evidence", "Status"],
                     ...
                     ["Objective 8-15 stock liquidity screen", "20-day turnover/volume/OI filter, sector and group caps, plus two declared stress overrides", "Partial"],
